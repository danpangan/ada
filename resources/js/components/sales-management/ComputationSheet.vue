    ...
    <div class="container">
        <div class="content-header mb-3" style="border-bottom: 1px solid black;">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h4 class="m-0 text-dark" v-show="isEdit"><strong>Edit Computation Sheet</strong></h4>
                        <h4 class="m-0 text-dark" v-show="!isEdit"><strong>View Computation Sheet</strong></h4>
                    </div>
                    <div class="col-sm-6">
                ...
                <form @submit.prevent="updateCS()">
                    <el-card class="box-card" shadow="hover">
                        ...
                        <el-form ref="form" :model="form" label-width="15%" :label-position="`right`">
                            <el-form-item label="Customer Name">
                                <el-col :span="5">
                                    <el-select v-model="form.cs.cs_client_id" placeholder="Customer Number" class="block" :disabled="true">
                                        <el-option
                                ...
                                <el-col :span="15" class="ml-1">
                                    <el-select v-model="form.cs.cs_client_id" placeholder="Customer Name" class="block" :disabled="true">
                                        <el-option
                                        v-for="client in clients"
                                        :key="client.id"
                    ...
                    <el-card class="box-card mt-3" shadow="hover">
                        ...
                        <el-form ref="form" :model="form" label-width="15%" :label-position="`right`">
                            <el-form-item label="Part Number">
                                <el-col :span="12">
                                    <el-select v-model="form.cs.client_order_id" placeholder="Part Number" class="block" @change="selectCSOrder"  @visible-change="getCSOrder" :disabled="true">
                                        <el-option
                            ...
                            <el-form-item label="Order Quantity">
                                <el-col :span="4">
                                    <el-input v-model="form.cs.cs_qty" :disabled="readonly"></el-input>
                                </el-col>
                                <el-col :span="4">
                            ...
                            <el-form-item label="Order Currency">
                                <el-col :span="24">
                                    <el-radio v-model="form.cs.cs_currency" @change="changeCurrency" :disabled="true" label="PHP">PHP</el-radio>
                                    <el-radio v-model="form.cs.cs_currency" @change="changeCurrency" :disabled="true" label="USD">USD</el-radio>
                                </el-col>
                            </el-form-item>
                        </el-form>
                    </el-card>
                        ...
                        <el-row>
                            <el-col :span="24">
                                <el-table
                                    ...
                                    <template slot-scope="scope">
                                        <el-col :span="24">
                                            <el-radio v-model="form.cs.cs_markup_percentage" :label="form.cs_markup[scope.$index].percentage" v-if="form.cs_markup[scope.$index].id < 5" :disabled="readonly">{{ form.cs_markup[scope.$index].percentage + ' %' }}</el-radio>
                                            <el-radio v-model="form.cs.cs_markup_percentage" :label="other_markup_percentage" :disabled="readonly" v-else>
                                                <el-select v-model="other_markup_percentage" placeholder="Other" size="small" style="width: 75%" @change="selectMarkup" :disabled="readonly">
                                                    <el-option
                    ...
                    <el-card class="box-card mt-3" shadow="hover">
                        ...
                        <el-row class="mt-3">
                            <el-col :span="12">
                                <el-table
                                    ...
                                    label="Name">
                                        <template slot-scope="scope">
                                            {{ form.cs_evaluation_checkers[scope.$index].first_name + ' ' + form.cs_evaluation_checkers[scope.$index].last_name }}
                                        </template>
                                    </el-table-column>
                                    ...
                                    <el-table-column label="Status" fixed="right" width="140" v-else>
                                        <template slot-scope="scope">
                                            <i :class="'icon '+evaluationStatusIcon(form.cs_evaluation_checkers[scope.$index].evaluation_status)+' ml-2'"></i>
                                            {{ evaluationStatus(form.cs_evaluation_checkers[scope.$index].evaluation_status) }}
                                        </template>
                                    </el-table-column>
                                </el-table>
                            </el-col>
                            <el-col :span="12">
                                <el-table
                                    ...
                                    label="Name">
                                        <template slot-scope="scope">
                                            {{ form.cs_evaluation_approvers[scope.$index].first_name + ' ' + form.cs_evaluation_approvers[scope.$index].last_name }}
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="Action" fixed="right" width="70" v-if="isEdit">
                                        <template slot-scope="scope">
                                    ...
                                    <el-table-column label="Status" fixed="right" width="140" v-else>
                                        <template slot-scope="scope">
                                            <i :class="'icon '+evaluationStatusIcon(form.cs_evaluation_approvers[scope.$index].evaluation_status)+' ml-2'"></i>
                                            {{ evaluationStatus(form.cs_evaluation_approvers[scope.$index].evaluation_status) }}
                                        </template>
                                    </el-table-column>
                                </el-table>
                            </el-col>
                        ...
                        <div v-show="!isEdit && evaluationForm.id" class="text-center">
                            ...
                            <br>
                            <el-button type="success" icon="el-icon-success" @click="evaluateCS" v-if="isEvaluated(evaluationForm.evaluation_status)" :disabled="includesRejected || isChecked || preventApproval || isApproved">{{ evaluationForm.evaluation_type == 1 ? 'Check' : 'Approve' }}</el-button>
                            ...
                            <el-button type="danger" icon="el-icon-remove-outline" @click="evaluateCS" :disabled="preventCheckDiscarding" v-else>Discard</el-button>
                        </div>
                    </el-card>

                    <div class="outside-card-button" v-if="isEdit">
                        <el-button type="primary" v-on:click="updateCS"><i class="el-icon-circle-check"></i> Update</el-button>
                        <el-button type="warning" v-on:click="resetFields"><i class="fas fa-file-pdf"></i> Preview</el-button>
                        <el-button type="info" @click="$router.back(-1)"><i class="el-icon-close"></i> Cancel</el-button>
                    </div>
                    <div class="outside-card-button" v-else>
                        <router-link :to="'/update-cs/'+this.$route.params.cs_id">
...
<script>
    ...
    export default {
        ...
        methods: {
            updateCS() {
                console.log(this.original_cs == this.form.cs)
                if(this.original_quantity == this.form.cs.cs_qty && this.original_total_cs == parseFloat(this.form.cs.cs_total_amount) && this.original_markup_percentage == this.form.cs.cs_markup_percentage){
                    Swal.fire({
                        text: 'Are you sure you want to update CS?',
                        icon: 'question',
                    ...
                    .then((result) => {
                        if(result.value){
                            this.isLoading = true;
                            this.form.put('/api/cs/'+this.$route.params.cs_id)
                            .then((res) => {
                                Swal.fire(
                                    'Updated!',
                    ...
                    let text = '';
                    if(this.original_quantity != this.form.cs.cs_qty) text = 'Order quantity was changed, CS update is not applicable for this condition. Are you sure you want to register new CS?';
                    else if(this.original_total_cs != parseFloat(this.form.cs.cs_total_amount)){
                        text = 'Order total amount was changed, CS update is not applicable for this condition. Are you sure you want to register new CS version?';
                        this.form.is_new_version = true;
                    }
                    else if(this.original_markup_percentage != this.form.cs.cs_markup_percentage){
                        text = 'Order markup was changed, CS update is not applicable for this condition. Are you sure you want to register new CS version?';
                        this.form.is_new_version = true;
                    }
                    ...
                    .then((result) => {
                        if(result.value){
                            ...
                            .then((res) => {
                                Swal.fire(
                                    'Registered!',
                                    'CS successfully registered. <br>CS Control No.: '+ res.data.cs_control_number,
                                    'success'
                                )
                            })
                            .catch(error => {
                                ...
                                else error_message = 'CS registration failed.';
                            ...
                            this.form.is_new_version = false;
                        }
                    });
                }
            },
            getCS(){
                ...
                .then((res) => {
                    ...
                    this.form.cs_evaluation_checkers =  res.data.cs_evaluation_checkers;
                    this.form.cs_evaluation_approvers =  res.data.cs_evaluation_approvers;
                    ...
                    this.evaluation_remarks = res.data.cs_evaluation_remarks;
                    this.evaluationForm.evaluator_id = res.data.user_id;

                    for (let i = 0; i < this.form.cs_evaluation_checkers.length; i++) {
                        if(this.form.cs_evaluation_checkers[i].evaluator_id == res.data.user_id){
                            this.evaluationForm.id = this.form.cs_evaluation_checkers[i].id;
                            this.evaluationForm.cs_id = this.form.cs_evaluation_checkers[i].cs_id;
                            this.evaluationForm.message = this.form.cs_evaluation_checkers[i].evaluator_remarks;
                            this.evaluationForm.evaluation_type = this.form.cs_evaluation_checkers[i].evaluation_type;
                            this.evaluationForm.evaluation_status = this.form.cs_evaluation_checkers[i].evaluation_status;
                        }
                    }
                    for (let i = 0; i < this.form.cs_evaluation_approvers.length; i++) {
                        if(this.form.cs_evaluation_approvers[i].evaluator_id == res.data.user_id){
                            this.evaluationForm.id = this.form.cs_evaluation_approvers[i].id;
                            this.evaluationForm.cs_id = this.form.cs_evaluation_approvers[i].cs_id;
                            this.evaluationForm.message = this.form.cs_evaluation_approvers[i].evaluator_remarks;
                            this.evaluationForm.evaluation_type = this.form.cs_evaluation_approvers[i].evaluation_type;
                            this.evaluationForm.evaluation_status = this.form.cs_evaluation_approvers[i].evaluation_status;
                        }
                    }
            ...
            exportPDF(){
                var cs_id = this.$route.params.cs_id;
                window.open('/export-pdf-cs?cs_id='+cs_id);
            },
            toEdit(){
                this.isEdit = true;
            },
                ...
                .then((result) => {
                    if(result.value){
                        if(!_.find(this.form.cs_evaluation_checkers, { 'evaluator_id': this.draft_evaluator_checker }) && !_.find(this.form.cs_evaluation_approvers, { 'evaluator_id': this.draft_evaluator_checker })){
                            ...
                            let id = 1;
                            if(this.form.cs_evaluation_checkers.length > 0){
                                id = this.form.cs_evaluation_checkers[this.form.cs_evaluation_checkers.length - 1].id + 1;
                            }
            ...
            evaluateCS(){
                Swal.fire({
                    text: 'Are you sure you want to evaluate CS?',
                    icon: 'question',
                    showCancelButton: true,
                ...
                .then((result) => {
                    if(result.value){
                        this.evaluationForm.put('/api/evaluate-cs')
                        .then((res) => {
                            this.getCS();
            ...
            rejectCSEvaluation(){
                ...
                Swal.fire({
                    text: 'Are you sure you want to '+process+' CS?',
                    icon: 'question',
                    showCancelButton: true,
        ...
        computed: {
            ...
            includesRejected(){
                let checkers_array = _.map(this.form.cs_evaluation_checkers, 'evaluation_status');
                let approvers_array = _.map(this.form.cs_evaluation_approvers, 'evaluation_status');
