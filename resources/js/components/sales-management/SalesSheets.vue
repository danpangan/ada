        ...
        <div class="content-header mb-3" style="border-bottom: 1px solid black;">
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <router-link :to="'/add-ss'">
                            <button type="submit" class="btn btn-add btn-sm float-right">
                                <i class="fas fa-plus mr-1"></i> Add New SS
                            </button>
                        </router-link>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header card-adamay-blue">
                        <h3 class="card-title">Sales Sheets</h3>
                    </div>
                    <div class="card-body">
                        ...
                        <el-table
                            ...
                            <el-table-column
                                ...
                                <template slot-scope="scope">
                                    {{ supplierCategory(SSTable[scope.$index].supplier_category) }}
                                </template>
                            </el-table-column>
                            <el-table-column
                                ...
                                label="Column">
                                <template slot-scope="scope">
                                    {{ supplierClassification(SSTable[scope.$index].supplier_classification_id) }}
                                </template>
                            </el-table-column>
                            <el-table-column
                                label="Action"
                                ...
                                <template slot-scope="scope">
                                    <el-tooltip class="item" effect="dark" content="More info" placement="top">
                                        <router-link :to="'/view-ss/'+SSTable[scope.$index].id">
                                    ...
                                    <el-tooltip class="item" effect="dark" content="Edit" placement="top" v-show="!showDeleted">
                                        <router-link :to="'/update-ss/'+SSTable[scope.$index].id">
                                    ...
                                    <el-tooltip class="item" effect="dark" content="Delete" placement="top" v-if="!showDeleted">
                                        <i class="icon el-icon-delete pointer ml-2" @click="deleteSupplier(SSTable[scope.$index].id)"></i>
                                    </el-tooltip>
                                    <el-tooltip class="item" effect="dark" content="Restore" placement="top" v-else>
                                        <i class="icon el-icon-circle-check pointer ml-2" @click="deleteSupplier(SSTable[scope.$index].id)"></i>
                                    </el-tooltip>
                                </template>
                            </el-table-column>
                        </el-table>
                        ...
                        <el-col class="show-deleted-checkbox" :span="6">
                            <el-checkbox v-model="showDeleted" @change="showDeletedOrders">Show deleted sales sheets</el-checkbox>
                        </el-col>
                    </div>
        ...
        methods: {
            getSS(){
                ...
                .then((data) => {
                    ...
                    if(this.$currentPageSS > 0){
                        this.handleCurrentChange(this.$currentPageSS);
                    }
                    else{
                        this.SSTable = data.data.data;
                        this.pageInfo = data.data;
                        this.pageInfo.current_page = this.$currentPageSS;
                        this.loading = false;   
                    }      
                })
                .catch(error => {
            ...
            deleteSupplier(ss_id){
                ...
                .then((result) => {
                    if(result.value){
                        this.loading = true;
                        axios.delete('api/sales-sheet/'+ss_id)
                        .then((data) => {
                            Swal.fire(
                                'Success!',
                                'Sales sheet has been '+this.alert_text+'d.',
                                'success'
                            )
                            this.getSS();
                            this.loading = false;
                        ...
                        .catch((error) => {
                            Swal.fire(
                                'Failed!',
                                'Failed to '+this.alert_text+' sales sheet.',
                                'warning'
                            )
                            this.loading = false;
                        })
            ...
            exportPDF(){
                ...
                window.open('/export-pdf-sales-sheets?keywords='+keywords+'&showDeleted='+isDeleted+'&sortProp='+sortProp+'&sortOrder='+sortOrder);
            },
            sortChange(param){
            ...
            currentPageGlobal(){
                Vue.prototype.$currentPageSS = this.pageInfo.current_page;
